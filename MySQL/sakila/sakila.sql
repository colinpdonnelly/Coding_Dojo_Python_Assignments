...
SELECT actor.actor_id, film.title, film.release_year, film.rating, film.special_features LIKE '%behind the scenes%' AS behind_the_scenes
FROM film
JOIN actor ON film.film_id = actor.actor_id
WHERE film.rating = 'G' OR actor.actor_id = 15;
...
-- Your query should return film title, description, release year, rating, special features, and genre (category).
SELECT film.title, film.description, film.release_year, film.release_year, film.rating, film.special_features, category.name, payment.amount
FROM film
...
SELECT film.title, film.description, film.release_year, film.rating, film.special_features, category.name, CONCAT_WS(' ', actor.first_name, actor.last_name) AS full_name
FROM actor
